...
- `banyan_mcp_rag_search`: call Banyan MCP `banyan.rag.search` for audited cross-corpus RAG citations from knowledge, posts, group-space posts, and DSH Skills with quality scoring, rubric dimensions, required corpus coverage, failure reasons, and tuning suggestions.
