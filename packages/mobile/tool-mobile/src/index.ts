...
function registerApkInstall(ctx: Context, timeoutMs: number): void {
  ctx.tools.register(defineTool({
    name: 'apk_install',
    description: 'Open Android system package installer for an APK that is accessible to Banyan. This does not silently install the APK; the user must confirm the Android installer prompt. Prefer this over coordinate tapping inside system install dialogs.',
    parameters: {
      filePath: { type: 'string', description: 'APK file path in the Banyan app private files directory.' },
...
function registerScreenScreenshot(ctx: Context, timeoutMs: number): void {
  ctx.tools.register(defineTool({
    ...
    description: 'Capture the current Android screen through the Banyan Android bridge and return it as a durable image attachment when attachments are available.',
    parameters: {},
    output: {
      schema: OUTPUT_SCHEMA,
      render: (_args, value) => renderScreenObserveOutput(value),
