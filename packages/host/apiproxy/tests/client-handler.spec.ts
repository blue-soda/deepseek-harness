...
function scriptedApi(overrides: {
  ...
  return {
    ...
    host: {
      ...
      copyDirectory: r => ok(r, {
        sourcePath: r.payload.sourcePath,
        targetPath: r.payload.targetPath,
        copiedFiles: 0,
        copiedDirectories: 0,
        skippedEntries: 0,
      }),
      installBanyanSkillPackage: r => ok(r, {
        targetRootPath: r.payload.targetRootPath ?? '/h/.dsh/skills',
        installedPath: `${r.payload.targetRootPath ?? '/h/.dsh/skills'}/${r.payload.directoryName}`,
